...
import EditSportForm from './edit-sport'
import { ISport } from '@/interfaces/sport'

export const sportFilterInput: IFilterInput[] = [
  {
    key: 'name',
    title: 'Name',
...
export const columns: ColumnDef<ISport>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
    enableHiding: true,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const sportId = row.getValue('id') as string
      return (
        <ActionButtonShow
          path={PATH_NAME.SPORT}
          id={sportId}
          tableKey='sports'
          editContentElement={<EditSportForm sportId={sportId} />}
          deleteMethod={() => sportApi.deleteSportById(sportId)}
        />
      )
    },
  },
